/*
  ...
  1. New Tables
    - `quotations` - User-generated freight quotations with full charge breakdown
      - Linked to user, schedule, carrier
      - Stores cargo details (container type, quantity, weight, commodity)
      - Stores totals by charge category (freight, surcharges, local origin, local dest)
      - Status lifecycle: draft → sent → accepted/rejected/expired
    - `quotation_charges` - Individual line items for each quotation
      - Linked to quotation with charge type (freight/surcharge/local_origin/local_destination)
    - `bookings_from_quotes` - Confirmed bookings created from accepted quotations
      - Stores shipper, consignee, cargo, and operational details
  2. Security
    - Users can only view/create/update their own quotations and bookings
    - RLS enforces user_id = auth.uid() ownership check
*/
